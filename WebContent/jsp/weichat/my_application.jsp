		...
		<div class="page">
			<div class="mt10 mb10">
				<ul class="list-activity my-activity">
					<c:forEach items="${pagingData.content }" var="application">
						<li>
							<a href="${pageContext.request.contextPath }/weichatActivity/view/01/${application.activity.category.code}/${application.activity.id}">
								<div class="fr text-center">
									<span class="badge bg-purple br50"><i class="icon-calendar"></i></span>
									<p class="gray"><small>${application.activity.applyNumber }报名</small></p>
								</div>
								<div class="text">
									<h3 class="ellipsis">${application.activity.name}</h3>
									<div class="status">
										<div class="gray community-name"><small>${application.activity.govOrg.name}</small></div>
										<div class="act-status"><small class="gray">报名状态：</small><small class="light-orange">${application.status.name}</small></div>
									</div>
								</div>
							</a>
						</li>
...
<div id="navigation" style="display: none;">
	<c:if test="${not empty nextParam}">
   		<a href="${pageContext.request.contextPath }/weichatShowMyApplications?${nextParam}">下一页</a>
   	</c:if>
</div>
</body>
</html>