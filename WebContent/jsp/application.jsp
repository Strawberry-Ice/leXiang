...
var flag;
			...
			jQuery(function($) {
				...
				jQuery(grid_selector).jqGrid({
					url:'${pageContext.request.contextPath }/activity/applyGrid/${code}',
					datatype: "json",
					height:  $(document).height()-360,
					colNames:[ '序号','活动名称', '报名人员','联系电话','报名日期','状态','审核','',''],
					colModel:[
						
						{name:'seq',index:'seq', width:60, sorttype:"int", editable: true,search:false},
						{name:'activityName',index:'activity.name',width:90, editable:true},
						{name:'applyName',index:'applyName', width:70, editable: true,edittype:"textarea"},
						{name:'applyTel',index:'applyTel', width:150, sortable:false,editable: true},
						{name:'applyDate',index:'applyDate', width:150,editable: true,sorttype:"date",search:false},
						{name:'status',index:'status', width:150, sortable:false,editable: true,sorttype:"date",search:false},
						...
						{name:'activityId',index:'activityId',hidden: true,search:false},
			...
			function agree(obj){
				...
				if(confirm("确定要同意吗？")){
					$.ajax({
						url : '${pageContext.request.contextPath }/weichatActivity/agree/'+rid,
						cache : false,
						dataType : "json",
						success : function(r) {
			...
			function reject(obj){
				...
				if(confirm("确定要拒绝吗？")){
					$.ajax({
						url : '${pageContext.request.contextPath }/weichatActivity/reject/'+rid,
						cache : false,
						dataType : "json",
						success : function(r) {
							$($t).trigger("reloadGrid", [{current:true}]);
