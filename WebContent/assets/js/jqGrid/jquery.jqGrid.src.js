...
//				$actionsDiv.find("div.ui-inline-save,div.ui-inline-cancel").show();
			...
			case 'agreeForJfdh' :
				if(confirm("确定要清空数据吗？")){
					$.ajax({
						url : '${pageContext.request.contextPath }/activity/agree/'+rid,
						cache : false,
						dataType : "json",
						success : function(r) {
	...
	//add by wangfei
	$.fn.fmatter.actionsForwf = function(cellval,opts) {
		var op={keys:false, editbutton:true, delbutton:true, editformbutton: false},
			rowid=opts.rowId, str="",ocl;
		if(opts.colModel.formatoptions !== undefined) {
			op = $.extend(op,opts.colModel.formatoptions);
		...
		if(op.editformbutton){
			...
			ocl = "id='jEditButton_"+rowid+"' onclick=jQuery.fn.fmatter.rowactions.call(this,'edit'); onmouseover=jQuery(this).addClass('ui-state-hover'); onmouseout=jQuery(this).removeClass('ui-state-hover') ";
			str += "<div title='"+$.jgrid.nav.edittitle+"' style='float:left;cursor:pointer;display:none;' class='ui-pg-div ui-inline-edit' "+ocl+"><span class='ui-icon ui-icon-pencil'></span></div>";
		}
		if(op.delbutton) {
			ocl = "id='jDeleteButton_"+rowid+"' onclick=jQuery.fn.fmatter.rowactions.call(this,'del'); onmouseover=jQuery(this).addClass('ui-state-hover'); onmouseout=jQuery(this).removeClass('ui-state-hover'); ";
			str += "<div title='"+$.jgrid.nav.deltitle+"' style='float:left;margin-left:5px;' class='ui-pg-div ui-inline-del' "+ocl+"><span class='ui-icon ui-icon-trash'></span></div>";
		}
		ocl = "id='jSaveButton_"+rowid+"' onclick=agree(this); onmouseover=jQuery(this).addClass('ui-state-hover'); onmouseout=jQuery(this).removeClass('ui-state-hover'); ";
		str += "<div title='"+$.jgrid.edit.bSubmit+"' style='float:left;' class='ui-pg-div ui-inline-save' "+ocl+"><span class='ui-icon ui-icon-disk'></span></div>";
		ocl = "id='jCancelButton_"+rowid+"' onclick=reject(this); onmouseover=jQuery(this).addClass('ui-state-hover'); onmouseout=jQuery(this).removeClass('ui-state-hover'); ";
	...
	$.fn.fmatter.applicationForwf = function(cellval,opts) {
		if(cellval==''){
			var op={keys:false, editbutton:true, delbutton:true, editformbutton: false},
				rowid=opts.rowId, str="",ocl;
			if(opts.colModel.formatoptions !== undefined) {
...
$.jgrid.extend({
	...
	navGrid : function (elem, o, pEdit,pAdd,pDel,pSearch, pView,pEnabled,pNotEnabled) {
		console.info(elem);
		o = $.extend({
			edit: true,
			editicon: "ui-icon-pencil",
			excel: true,//add by wangfei
			excelicon: "fa-download",//add by wangfei
			send:true,
			sendicon:'fa-send',
			add: true,
			...
			enabled : false,
			enabledicon : "glyphicon-ok",
			notenabled:false,
			notenabledicon:" glyphicon-remove",
			position : "left",
			closeOnEscape : true,
			beforeRefresh : null,
			afterRefresh : null,
		...
		return this.each(function() {
			...
			for(i = 0; i<clone; i++) {
				...
				if (o.excel) {
					...
					$(tbd,navtbl)
					.attr({"title":o.exceltitle || "",id : pAdd.id || "excel_"+elemids})
					.click(function(){
						window.open(o.excelurl);
						return false;
				...
				if (o.send) {
					pAdd = pAdd || {};
					tbd = $("<td class='ui-pg-button ui-corner-all'></td>");
					$(tbd).append("<div class='ui-pg-div'><span class='ui-icon "+o.sendicon+"'></span>"+o.sendtext+"</div>");
					$("tr",navtbl).append(tbd);
					$(tbd,navtbl)
					.attr({"title":o.sendtitle || "",id : pAdd.id || "excel_"+elemids})
					.click(function(){
						if (!$(this).hasClass('ui-state-disabled')) {
							var dr;
							if($t.p.multiselect) {
...
//						}
//						return false;
						location.href=pAdd.url;//edit by wangfei
...
//							$("a.ui-jqdialog-titlebar-close").click();
				...
				if (o.enabled) {
					tbd = $("<td class='ui-pg-button ui-corner-all'></td>");
					pEnabled = pEnabled || {};
					$(tbd).append("<div class='ui-pg-div'><span class='ui-icon "+o.enabledicon+"'></span>"+o.enabledtext+"</div>");
					$("tr",navtbl).append(tbd);
					$(tbd,navtbl)
					.attr({"title":o.enabledtitle || "",id: pEnabled.id || "del_"+elemids})
					.click(function(){
						if (!$(this).hasClass('ui-state-disabled')) {
							var dr;
							if($t.p.multiselect) {
				...
				if (o.notenabled) {
					tbd = $("<td class='ui-pg-button ui-corner-all'></td>");
					pNotEnabled = pNotEnabled || {};
					$(tbd).append("<div class='ui-pg-div'><span class='ui-icon "+o.notenabledicon+"'></span>"+o.notenabledtext+"</div>");
					$("tr",navtbl).append(tbd);
					$(tbd,navtbl)
					.attr({"title":o.notenabledtitle || "",id: pNotEnabled.id || "del_"+elemids})
					.click(function(){
						if (!$(this).hasClass('ui-state-disabled')) {
							var dr;
							if($t.p.multiselect) {
