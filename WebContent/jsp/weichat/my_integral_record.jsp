...
<body>
	<nav id="head" class="header fixed-top" style="background-color: white;">
		<ul class="nav-primary nav-primary-mix">
			...
			<li><a href="${pageContext.request.contextPath}/weichatScoreShop/findAllMyScoreRecords2"><i class="icon-cart2"></i><p>积分排行</p> </a> </li>
		</ul>
	</nav>
			...
			<div class="mt10 mb10">
				<ol class="line-list ol-num">
					<c:forEach items="${pagingData.content }" var="record">
						<li class="<c:choose><c:when test="${record.plus }">add</c:when><c:otherwise>minus</c:otherwise></c:choose> show_info">
							<span class="fr text-center gray"><small><fmt:formatDate value="${record.createDate }" type="date" pattern="yyyy-MM-dd"/></small></span>
							<span class="personname"><a href="${pageContext.request.contextPath }/weichatScoreShop/myRecord/${record.id}">${record.message }</a></span>
							<span class="score"><c:choose><c:when test="${record.plus }">+</c:when><c:otherwise>-</c:otherwise></c:choose>${record.score }分</span>
						</li>
					</c:forEach>
				</ol>
			</div>
	...
	<div id="navigation" style="display: none;">
		<c:if test="${not empty nextParam}">
	   		<a href="${pageContext.request.contextPath }/weichatScoreShop/findAllMyScoreRecords?${nextParam}">下一页</a>
	   	</c:if>
	</div>
</body>
</script>
